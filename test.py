...
async def run_in_executor(func, *args, **kwargs):
    """Helper function to run blocking calls asynchronously."""
...
async def read_events():
    ...
    try:
        while True:
            ...
            list_request = events_service.list(
...
async def edit_event():
    """API endpoint to edit an existing event in a specified Google Calendar."""
    service = get_calendar_service()
    data = await request.get_json()
...
async def query_database(database_id):
    """Query a Notion database and return the pages."""
    try:
...
async def create_notion_entry():
    """API endpoint to create a new entry in a specified Notion database."""
    data = await request.get_json()
    database_id = data.get("database_id")
    properties = data.get("properties")
    content = data.get("content")  # Content blocks for the page
...
async def query_notion_database():
    """API endpoint to query a Notion database with filters and return matching results."""
    data = await request.get_json()
    database_id = data.get("database_id")
